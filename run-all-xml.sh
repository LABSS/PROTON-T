...
version="rp0.8"
NLOGO="/home/mario/NetLogo\ 6.1.1/netlogo-headless4_G.sh" # labss-simul
NLOGO="/home/paolucci/NetLogo\ 6.1.1/netlogo-headless.sh" # tiny
CPU_FREE=25 # the free cpu needed to launch a new one. For 2-sims, it should mean we have two free processors, so 
# 25% for tiny, 2/32 is 0.06 so maybe 10? 15? 


# checks every ten seconds
wait_until_cpu_free_atleast() {
    awk -v target="$1" 'NR > 3 {print $NF;  if($NF  >= target) { exit(0); }} ' < <(LC_ALL=C mpstat 10 --dec=0)
 }
...
for arg in `ls experiments-xml/*rp0.8.[a-z].xml`; do
   ...
   sleep 20 # it should be long enough to complete a 40k setup
done
